"""Frontier exploration driven by a VLM instead of A* - the measured control for explore.py.
    ...
    python slamtest/drivers/explore_vlm.py --max-steps 20                     # VLM smoke test
    python slamtest/drivers/explore_vlm.py --max-steps 300                    # full VLM run
...
----------------------
--output-dir defaults to slamtest/output_vlm/<run-tag>/ - a PER-TAG subdirectory (tag defaults to
the --planner value), never slamtest/output. Two reasons, both load-bearing:
...
_THIS_DIR = os.path.dirname(os.path.abspath(__file__))        # slamtest/drivers
...
DEFAULT_OUTPUT_ROOT = os.path.join(_SLAM_DIR, "output_vlm")

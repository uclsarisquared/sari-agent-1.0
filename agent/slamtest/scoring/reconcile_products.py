...
Design stances, and why:
  ...
  * OFFLINE, DETERMINISTIC, FREE. Pure string algebra over two JSONs; no model, no sim. The
    ...
    python slamtest/scoring/reconcile_products.py                    # writes *_reconciled.json + report
    python slamtest/scoring/reconcile_products.py --self-test        # calibration cases only, no write
"""
import argparse
import difflib
import json
...
_THIS_DIR = os.path.dirname(os.path.abspath(__file__))        # slamtest/scoring
...
def main():
    ...
    ap.add_argument("--output-dir", default=os.path.join(_SLAM_DIR, "output"))

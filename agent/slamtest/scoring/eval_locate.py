...
view from a checkpoint + an honest verdict. No manipulation, no centring, no delivery.
    ...
    python slamtest/scoring/eval_locate.py                     # zoom ladder ON (default)
    python slamtest/scoring/eval_locate.py --no-zoom           # ablation arm
    python slamtest/scoring/eval_locate.py --only slang --backend claude-cli
"""
import argparse
import json
import os
...
def main():
    ...
    run_dir = args.run_dir or os.path.join(
        _SLAM_DIR, "output", "locate_trials",

...
Design notes (why it looks the way it does):
  ...
  * The OFFLINE annotate_pass stays the primary prompt-iteration tool (saved PNGs, no sim); this
    ...
    python slamtest/capture/capture_annotate_walk.py slamtest/output --limit 0 --angles 2

    # resume an interrupted fused run (skips finished checkpoints on both sides):
    python slamtest/capture/capture_annotate_walk.py slamtest/output --limit 0 --angles 2 --resume
"""
import json
import os
import sys
...
_THIS_DIR = os.path.dirname(os.path.abspath(__file__))        # slamtest/capture

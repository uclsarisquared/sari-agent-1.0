...
agent** — that lives in `agent/` (entry: `subtask_agents.py`). Kept for reference and
...
`agent/` — both stacks define their own `env.py`/`actions.py`, which is why the code was never
shared via `sys.path`.

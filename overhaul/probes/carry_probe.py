...
Usage:
    python probes/carry_probe.py                       # grab at cp32, auto 4-leg route ending at the counter, 3 routes
    python probes/carry_probe.py --grab-cp 32 --route 15,20,32,54 --routes 3
    python probes/carry_probe.py --legs 4 --routes 3   # auto route of 4 legs, repeated 3 times
...
def run(args):
    ...
    run_dir = os.path.join(_SLAM, "output", "gate6_1_out", datetime.now().strftime("%m%d_%H%M%S"))

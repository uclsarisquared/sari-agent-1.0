...
Start ``uv run poe ocr-server`` first, then run:
    ...
    uv run python overhaul/probes/ocr_service_smoke.py receipt.png --clients 20 --pid <server-pid>
...
if str(_OVERHAUL) not in sys.path:
    sys.path.insert(0, str(_OVERHAUL))

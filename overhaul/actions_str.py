...
MANIPULATION_ACTIONS = (
    ...
    "checkout_held_item: Emit THIS to check out a held item in ONE deterministic step - it drives to the self-checkout counter, squares onto the scan pad, scans the held item, and bags it in the tray. The hand is chosen automatically: the one holding an item (LEFT first when both hold - use checkout_held_item_right to check out the right hand's item instead). Do NOT hand-sequence the driving / aligning / scanning / placing yourself - this single action does all of it (times value is ignored). Works ONLY in *manipulation* mode, and ONLY when the CURRENT GOAL is a checkout/scan-and-bag subtask - if your goal is merely to pick something up, do NOT check out; finish by emitting STOP and a later agent handles the checkout. Afterwards, read `last_checkout` in the state: STOP only once it shows scanned AND placed; if it did not scan or bag, re-emit checkout_held_item or reposition per its reason. When BOTH hands hold items, check out one at a time (each call scans and bags ONE item).\n"

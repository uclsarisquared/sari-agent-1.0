...
into subtask_agents.
    ...
    python plan6/test_files/ab_decompose.py                 # all prompts, both arms
    python plan6/test_files/ab_decompose.py --arm new        # only the typed arm
    python plan6/test_files/ab_decompose.py --only f3-size   # a single battery id

It needs the UCL qwen server (same reasoner the orchestrator uses) - importing agent/subtask_agents
pulls the model stack, so first import is slow; the LLM calls need the vLLM server reachable. Writes
plan6/test_files/ab_decompose_out.json for the record and prints a per-prompt diff + a summary of how
...
from subtask_completion import TYPED_DECOMPOSER_SYSTEM, parse_decomposition, SUBTASK_TYPES

_HERE = os.path.dirname(os.path.abspath(__file__))
_BATTERY = os.path.join(_HERE, "decompose_battery.json")
_OUT = os.path.join(_HERE, "ab_decompose_out.json")
...
def main():
    ...
    from subtask_agents import _llm_client, _llm_call
    client = _llm_client()

...
Usage:
    python plan6/carry_probe.py                       # grab at cp32, auto 4-leg route ending at the counter, 3 routes
    python plan6/carry_probe.py --grab-cp 32 --route 15,20,32,54 --routes 3
    python plan6/carry_probe.py --legs 4 --routes 3   # auto route of 4 legs, repeated 3 times
...
_ROOT = os.path.dirname(_THIS)                       # overhaul/
_SLAM = os.path.join(_ROOT, "slamtest")
for _p in (_ROOT, _SLAM):
...
def secure_grip(nav, uri):
    ...
    from env import (RequestLidarCenter, TransformHands, SetCrouch,
                     move_forward, move_backward, move_left, move_right,
                     pan_left, pan_right, tilt_up, tilt_down)
    from manipulation import extend_arm_until_grabbed, plan_reach
...
def run(args):
    ...
    from env import TransformHands, ToggleLeftGrip, RequestScreenshot
    ...
    run_dir = os.path.join(_THIS, "gate6_1_out", datetime.now().strftime("%m%d_%H%M%S"))

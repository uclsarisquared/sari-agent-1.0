...
def test_a4_findings_prompt_caps_the_retained_handoff(monkeypatch) -> None:
    ...
    monkeypatch.setattr(subtask_agents, "_llm_call", fake_call)
    result = subtask_agents.generate_findings_summary(
...
def test_a3_does_not_make_a_findings_call(monkeypatch) -> None:
    ...
    assert subtask_agents._generate_findings_if_enabled(

...
into subtask_agents.
    ...
    python tests/ab_decompose.py                 # all prompts, both arms
    python tests/ab_decompose.py --arm new        # only the typed arm
    python tests/ab_decompose.py --only f3-size   # a single battery id
...
tests/ab_decompose_out.json for the record and prints a per-prompt diff + a summary of how
...
def main():
    ...
    from orchestrator.subtask_agents import _llm_client, _llm_call
    client = _llm_client()

...
TYPED_DECOMPOSER_SYSTEM = (
    ...
    "any list.\n"
    "  - goto:     \"location\" = where to go. This is EITHER a place the task or store memory names "
    "('Checkpoint 32', 'the checkout counter', 'Aisle 2') OR THE NAME OF A PRODUCT, which means 'go "
    "to where that product is shelved' - the store map resolves a product name to its checkpoint(s) "
    "exactly as it does for a pickup target. Copy the place or product name as the task words it; "
    "never invent shelf numbers or location names.\n\n"
    ...
    "  - An inspect subtask whose query names a SPECIFIC PRODUCT must be PRECEDED BY A GOTO to that "
    "product. The agent does not start next to it, and an inspect subtask cannot travel - it would "
    "otherwise try to answer the question from wherever it happens to be standing. Omit that goto "
    "ONLY when the agent is already there: an earlier subtask in the same plan already named that "
    "product (a goto, pickup, or compare), or the task itself says the item is in view or in hand "
    "('on this shelf', 'here', 'the one I am holding'). So 'find X and read/check/report Y' is "
    "goto X + inspect Y, never a bare inspect.\n\n"
    ...
    "Example output: "
    "[{\"type\": \"goto\", \"location\": \"milk\", \"text\": \"Go to where the milk is shelved.\"}, "
    ...
    "Example output: "
    "[{\"type\": \"goto\", \"location\": \"Choco Mallows\", "
    "\"text\": \"Go to where the Choco Mallows are shelved.\"}, "
    "{\"type\": \"inspect\", \"query\": \"What is the printed price of the Choco Mallows?\", "
    "\"text\": \"Look at the Choco Mallows and report their printed price without touching them.\"}]"

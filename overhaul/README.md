...
the flat-module era (`from env import ...`) is gone. Run everything from `overhaul/` — entry
...
overhaul consumers (`nav.store_map`, `nav.locate_task`, `agent_core.memory_gen`) import it;
...
`episodic_memory.txt`, `semantic_memory.txt` (written by `agent_core.agent`).
...
🎮 = needs the Unity sim in Play mode (ws://localhost:8080). Run everything from `overhaul/`.

...
> The `plan6/` folder was dissolved in the 2026-07-24 reorg. Where things went: unit tests +
> `ab_decompose` → `tests/`; `carry_probe.py`, `step0_hand_pose_probe.py`,
> `live_correction_probe.py` → `probes/`; measurement outputs `gate6_1_out/`, `step0_out/` →
> `slamtest/output/`. `CHECKLIST.md` (the phase-6 progress/gate record) was **deleted on user
...
```
python plan6/step0_hand_pose_probe.py
```
...
Every read is appended to `plan6/step0_out/<run>/measurements.jsonl`; screenshots land beside it;
...
- **Step 4 — between-task reset**: `eval_pickup.return_to_start`'s stow logic is left intact (it
...
### Tests — [test_files/](test_files/) (offline, no sim)

```
python -m pytest plan6/test_files -q            # 16 tests
```

- [test_set_hand_pose.py](test_files/test_set_hand_pose.py) — convergence, over-clamp iteration,
  named + xyz poses, right-hand routing, frame-mismatch → `arrived=False`.
- [test_extend_arm_guard.py](test_files/test_extend_arm_guard.py) — full-hand guard refuses without
  touching the pose; GRAB-then-REST on success, miss, **and** exception.
- [test_agent_hand_pose_router.py](test_files/test_agent_hand_pose_router.py) — activate + drive
  REST; no-spam on repeat; invalidate forces a re-drive; hard-stow → re-activate + re-drive; router
  never sets GRAB.

## Step 5 — Gate 6.1: [carry_probe.py](carry_probe.py) 🎮

The scripted, no-LLM gate — run it to test the whole 6.1 chain live:

```
python plan6/carry_probe.py                       # grab @ cp32, auto route to the counter, 3 routes
python plan6/carry_probe.py --grab-cp 32 --route 15,20,32,54 --routes 3
```
...
Logs + shots land in `plan6/gate6_1_out/<run>/`. Only after this passes does agent-loop work start.

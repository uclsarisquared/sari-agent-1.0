...
view from a checkpoint + an honest verdict. No manipulation, no centring, no delivery.
    ...
    python slamtest/eval_locate.py                     # zoom ladder ON (default)
    python slamtest/eval_locate.py --no-zoom           # ablation arm
    python slamtest/eval_locate.py --only slang --backend claude-cli
"""
import argparse
import json
import os
...
_OVERHAUL = os.path.dirname(_THIS_DIR)
for _p in (_OVERHAUL, _THIS_DIR):
    if _p not in sys.path:
        sys.path.insert(0, _p)

from store_map import StoreMap, NavSession  # noqa: E402
import locate_task  # noqa: E402
...
def main():
    ...
    run_dir = args.run_dir or os.path.join(
        _THIS_DIR, "output", "locate_trials",

...
\ts. `last_checkout`: The result of the most recent `checkout_held_item` call - {scanned, placed, aligned, hand, reason, ...} - or None if it has not run this subtask. `scanned` and `placed` are MEASURED; `hand` is which hand's item it scanned and bagged (the other hand may still be carrying its own item - that one needs its own checkout). On a checkout subtask: STOP once both are True; if either is False, the `reason` says what failed - retry or reposition per it. Never claim the checkout happened while this is None.

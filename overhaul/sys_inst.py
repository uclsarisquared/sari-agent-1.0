...
case before believing. v1 REPORTS crouch only (SetCrouch is not auto-invoked yet, user directive).
"""
...
\ts. `last_checkout`: The result of the most recent `checkout_held_item` call - {scanned, placed, aligned, reason, ...} - or None if it has not run this subtask. `scanned` and `placed` are MEASURED. On a checkout subtask: STOP once both are True; if either is False, the `reason` says what failed - retry or reposition per it. Never claim the checkout happened while this is None.
\tt. `nearest_checkpoint`: The map checkpoint id closest to your current position (measured from your live pose). Useful to know where you are; on a go-to subtask, arrival is judged against this. (`visited_checkpoints` and `gripped_name` are related bookkeeping: checkpoints you have stood at this task, and the item name recorded at the moment of your grab.)"""

...
SYS_INST_ASSOCIATIVE_SEMANTIC = ("You are a Semantic Associative Learner that synthesizes new semantic memories for an Embodied AI Agent operating within a 3D convenience store simulation. "
                                 ...
                                 "'recall': (string) From your updated semantic memory, recall information relevant to the current observation and task — specifically: item locations, what is currently visible, and the target shelf identity and coordinates. Do NOT include path planning or navigation routing; that is handled separately by the navigation reasoner."
                                 ...
                                 "2. If you can directly see an obstruction like a shelf or wall, then suggest actions such that the agent does not collide (move towards/through) that obstruction. Instead, suggest that the agent should pan left/right to avoid the obstruction.\n"
                                 "**Mode-switching rules**:\n"
                                 "- Switch to *manipulation* when: `leftHoveredObject` or `rightHoveredObject` in the agent state matches the target item, OR the depth hint indicates the item is ≤1 meter away.\n"
                                 "- Switch to *STOP* when: the gripped item matches the task target AND (if the task requires placement) the agent is within 1 meter of the counter. Gripping alone is NOT sufficient for STOP if the task requires bringing the item somewhere.\n"
                                 "- Switch to *navigation* when the target item is not visible or the agent is not near the target shelf.\n"
                                 "- Switch to *perception* when the agent is near the target shelf but needs to visually confirm and center on the item.\n\n")
...
SYS_INST_ASSOCIATIVE_EPISODIC = ("You are an Episodic Associative Learner that focuses on generating episodic reflections for an Embodied AI Agent operating within a 3D convenience store simulation. "
                                 ...
                                 "```json\n"
                                 "{\n"
                                 "  'dense_summary': (string) A concise summary of the agent's actions and observations. Include the agent's position (x, z) at key moments — where it started, where it got stuck, and where it succeeded.,\n"
                                 "  'what_worked': (string) A reflection on effective strategies. Reference the specific aisle or coordinate range that successful paths used (e.g., 'routing through central aisle A at x≈2.0 from z=3.9 to z=7.6 worked well').,\n"
                                 "  'what_to_avoid': (string) A reflection on ineffective strategies. Reference specific coordinates where failures occurred (e.g., 'moving directly from x≈1.5 toward x≈4.4 at z≈3.9 causes a collision near x=3.8 — Shelf 3 east face'). Remind the agent to trust visual cues over numerical targets.\n"
                                 "}\n"
                                 "```\n\n")
...
SYS_INST_VLM_LEAN = ("You are an Embodied AI Agent operating within a 3D convenience store simulation. "
                     ...
                     "14. **Waypoint following**: When '## NAVIGATION WAYPOINT' appears in your input, treat it as your immediate sub-goal. Navigate to the specified (x, z) coordinate using the action hints provided before pursuing the final target. These waypoints were pre-computed by a global planner with access to the full store map.\n"

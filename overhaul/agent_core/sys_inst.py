...
\tt. `last_inspection`: The result of the restricted `inspect_held_item` macro, or None before it runs. The macro locks a side only when the requested label is directly front-facing, never merely readable at an angle, then runs PaddleOCR on centered item crops while moving closer. `label_legible=True` means the strict VLM found the requested nutritional/expiration label readable; read the exact requested values from the CURRENT screen, use `ocr_lines` as supporting transcription evidence, and emit STOP with them in `reported_answer`. `best_effort_read=True` means the front-facing label reached the closest allowed position but remained formally illegible; nevertheless use both the CURRENT screen and `ocr_lines` to transcribe the best answer possible and STOP. PaddleOCR is auxiliary and may include unrelated/noisy text, so reconcile it with the visible label rather than copying blindly. Do not run the macro again after either signal. `sweep_exhausted=True` means all five progressively closer X-then-Y search sweeps completed without finding the requested label directly front-facing on any side; do not invent values.

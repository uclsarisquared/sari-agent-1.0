"""smoke_checkout.py - one-item live smoke of the Phase 6.2 checkout chain (align -> scan -> bag). 🎮
...
macro and BEFORE the 5-run gate. This is the supervised dry-run the phase6.2 plan calls for: it is
where align_to_scanner's live margins and place_held_item's release show themselves.

    python smoke_checkout.py --grabnow       # grab from where you stand (you aligned it), then chain
...
def main():
    ...
    from perception import (scan_held_item, place_held_item,
                            center_to_screen, center_to_scanner, read_text_in_box)
    from store_map import StoreMap, NavSession, go_to_counter, align_to_scanner

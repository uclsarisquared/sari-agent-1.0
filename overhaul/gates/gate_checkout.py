...
whether it landed in the tray by eye). 🎮  Needs the sim in Play mode on ws://localhost:8080.
    ...
    python gate_checkout.py --two-hands     # grab one item in EACH hand, check BOTH out per run
...
Each run:
  ...
  4. a row PER ITEM is logged (measured + verified) to slamtest/output/placetests/gate_checkout.csv
     (single) or gate_checkout_2h.csv (--two-hands, with a `hand` column).
...
_ROOT = os.path.dirname(_THIS)  # overhaul/ — runtime modules (env, store_map) live there
if _ROOT not in sys.path:
    sys.path.insert(0, _ROOT)

CSV_PATH = os.path.join(_ROOT, "slamtest", "output", "placetests", "gate_checkout.csv")
...
CSV_PATH_2H = os.path.join(_ROOT, "slamtest", "output", "placetests", "gate_checkout_2h.csv")
...
def main():
        ...
        while True:                                      # manual path: user aligns, we grab in place
            try:
                cmd = input(f"  Run {run}: align the agent on an item for the {side} hand, then Enter "
                            f"to grab (q = abort gate) > ").strip().lower()
            except (EOFError, KeyboardInterrupt):
                return None
            if cmd == "q":
                return None
    ...
    def ensure_holding(run):
        """Fill the hand(s) this gate needs. Returns {side: item_name} or None to abort. Two-hand mode
    ...
    try:
        for run in range(1, args.runs + 1):
            print(f"\n{'=' * 70}\n  GATE RUN {run}/{args.runs}{'  (two-hand)' if two else ''}"
                  f"\n{'=' * 70}")
    ...
    n = len(run_summaries)
    print(f"\n{'=' * 70}\n  GATE 6.2 RESULT ({n} run(s){', two-hand' if two else ''})\n{'=' * 70}")
    if not n:
